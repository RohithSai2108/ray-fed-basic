...
# Note: get_partitioned_mnist returns (train_loader, test_loader) for the given client id
# using EMNIST split='mnist' internally (so it's identical to MNIST but downloads reliably).
...
class Client:
    ...
    def __init__(
        ...
        """
        cid: client id (0 .. num_clients-1)
        num_clients: total number of clients (used for partitioning)
        dataset: currently only 'mnist' supported (uses EMNIST split='mnist' under the hood)
        in_channels: model input channels
        malicious_id: client id to simulate malicious behaviour (optional)
        batch_size: local batch size used for the partitioned loader
        """
        self.cid = cid
        ...
        if ds == "mnist":
            # IMPORTANT: use partitioned loader so each client gets a unique shard
            self.train_loader, self.test_loader = get_partitioned_mnist(
                cid, num_clients, batch_size=batch_size
            )
        ...
        # Initialize prev_global from the model's current state (CPU clones)
        self.prev_global = {
    ...
    def set_global(self, state_dict):
        ...
        # state_dict is expected to be CPU tensors or compatible
        self.model.load_state_dict(state_dict)
        # store a CPU-cloned copy to compute deltas later
    ...
    def train(self, local_epochs: int = 1, score: float = 1.0, lr: float = 0.01, dp: bool = True):
        ...
        for e in range(local_epochs):
            ...
            # Use print for simple Ray logs; consider ray.get/logging if you want central logs
